...
namespace GAS.Editor.Stat
{
    ...
    {
        /// <summary> StatData 扫描缓存 </summary>
        private static List<StatData> cachedStatDataList;

        /// <summary> StatId 下拉缓存 </summary>
        private static readonly Dictionary<int, List<string>> statIdCacheDict =
        ...
        public const string BaseMaxValuePropertyName = "baseMaxValue";
        ...
        {
            if (cachedStatDataList != null)
                return new List<StatData>(cachedStatDataList);
        ...
        {
            int cacheKey = (passiveOnly ? 1 : 0) | (immediateOnly ? 2 : 0);
            if (statIdCacheDict.TryGetValue(cacheKey, out List<string> cachedIdList))
                return new List<string>(cachedIdList);
            ...
            statIdCacheDict[cacheKey] = idList;
            return idList;
        }
        ...
        {
            cachedStatDataList = null;
            statIdCacheDict.Clear();
        }
        ...
        public static StatData CreateStatData(string assetName, E_StatType statType, float baseValue = 100f)
        {
            ...
            so.FindProperty(BaseMaxValuePropertyName).floatValue = baseValue;
    ...
    /// <summary>
    /// StatData 资产变化后刷新编辑器缓存
    /// </summary>
    public sealed class GasStatAssetPostprocessor : AssetPostprocessor
    {
        private static void OnPostprocessAllAssets(
            string[] importedAssetPathList,
            string[] deletedAssetPathList,
            string[] movedAssetPathList,
            string[] movedFromAssetPathList)
        {
            if (HasStatAssetPath(importedAssetPathList)
                || HasStatAssetPath(deletedAssetPathList)
                || HasStatAssetPath(movedAssetPathList)
                || HasStatAssetPath(movedFromAssetPathList))
            {
                GasStatEditorUtility.ClearCache();
            }
        }

        private static bool HasStatAssetPath(string[] assetPathList)
        {
            ...
            {
                if (assetPathList[i].EndsWith(".asset", StringComparison.OrdinalIgnoreCase))
                    return true;
            }
