...
namespace GAS.Core
{
    ...
    {
        /// <summary> 压测时置 true 关闭 Log </summary>
        ...
        /// 移除指定运行时 GE
        /// </summary>
        public void RemoveGE(GameplayEffectRuntime runtime)
        {
            if (runtime == null)
                return;

            int index = appliedEffectList.IndexOf(runtime);
            if (index >= 0)
                RemoveGameplayEffect(runtime, index);
        }
        ...
        {
            return ApplyGE(effectData, source, null);
        }
            ...
            IReadOnlyDictionary<string, float> valueOverrideDict)
        {
            ...
            {
                GameplayEffectRuntime instantRuntime = new GameplayEffectRuntime(
                    effectData,
                    source,
                    valueOverrideDict);
            ...
            if (effectData.IsIndependentStack)
                return ApplyIndependent(effectData, source, valueOverrideDict);
            ...
            return AddNewEffect(effectData, source, valueOverrideDict);
        }
        ...
        /// </summary>
        private GameplayEffectRuntime ApplyIndependent(
            GameplayEffectData effectData,
            object source,
            IReadOnlyDictionary<string, float> valueOverrideDict)
        {
            ...
            for (int i = 0; i < toAdd; i++)
                lastRuntime = AddNewEffect(effectData, source, valueOverrideDict);
        ...
        /// 新建并挂上列表
        /// </summary>
        private GameplayEffectRuntime AddNewEffect(
            GameplayEffectData effectData,
            object source,
            IReadOnlyDictionary<string, float> valueOverrideDict = null)
        {
            GameplayEffectRuntime runtime = new GameplayEffectRuntime(
                effectData,
                source,
                valueOverrideDict);
        ...
        private void ApplyPeriodicEffect(GameplayEffectRuntime spec)
        {
            ...
            for (int i = 0; i < configList.Count; i++)
            {
                ...
                if (config.isMaxValueModifier)
                    continue;

                float finalValue = spec.GetModifierValue(config) * stackCount;
        ...
        private void ApplyModifiersToStat(GameplayEffectRuntime spec, bool isRemove = false)
        {
            ...
            for (int i = 0; i < configList.Count; i++)
            {
                ...
                if (isRemove)
                {
                    // 即时属性上限修饰 通常只挂一次 如降低最大生命
                    if (config.isMaxValueModifier)
                    {
                        statController.RemoveMaxModifiersFromSource(config.statId, spec);
                        continue;
                    }

                    // 即时属性瞬时改值无长期挂载 被动属性按来源清修饰符
                    if (statController.GetImStat(config.statId) != null)
                        continue;
                    statController.RemoveModifiersFromSource(config.statId, spec);
                    continue;
                }

                // 先算 finalValue = 配置值 × StackCount 得到最终值
                float finalValue = spec.GetModifierValue(config) * spec.StackCount;

                // 即时属性改上限
                if (config.isMaxValueModifier)
                {
                    statController.AddMaxModifier(config.statId, new StatModifier(
                        string.Empty,
                        config.type,
                        finalValue,
                        spec,
                ...
                if (statController.GetImStat(config.statId) != null)
                {
                    ...
                    statController.ChangeAttributeValue(config.statId, finalValue, config.type, spec);
                    continue;
                }

                // 被动属性挂长期修饰符
                statController.AddModifier(config.statId, new StatModifier(
                    string.Empty,
                    config.type,
                    finalValue,
                    spec,
