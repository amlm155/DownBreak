...
namespace GAS.Core.GameplayEffect
{
    ...
    public class GameplayEffectRuntime
    {
        ...
        /// <summary> 运行时覆盖的属性修饰值 </summary>
        private readonly IReadOnlyDictionary<string, float> valueOverrideDict;

        /// <summary>
        /// 构造运行时实例
        /// </summary>
        public GameplayEffectRuntime(
            GameplayEffectData definition,
            object source,
            IReadOnlyDictionary<string, float> valueOverrideDict = null)
        {
            GEData = definition;
            Source = source;
            this.valueOverrideDict = valueOverrideDict;
        ...
        /// <summary>
        /// 获取属性修饰值 运行时覆盖优先于资产默认值
        /// </summary>
        public float GetModifierValue(StatModifierConfig config)
        {
            if (valueOverrideDict != null
                && valueOverrideDict.TryGetValue(config.statId, out float overrideValue))
            {
